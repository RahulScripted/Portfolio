...
import { careerEntries } from "../data";
...
export default function CareerLedger() {
  return (
    <section id="career" className="py-[76px] px-5 sm:px-[30px]">
        ...
        <div className="border-t-2 border-ink">
          {careerEntries.map((entry, i) => (
            ...
            >
              {/* Date + type */}
              <div>
                <span className="font-mono text-[13px] text-ink-soft block">{entry.dateRange}</span>
                <span
                ...
                {entry.contributions.length > 0 && (
                  <ul className="mt-3 space-y-1">
                    {entry.contributions.map((c, ci) => (
                      <li key={ci} className="font-mono text-[11px] text-ink-soft flex items-start gap-2">
